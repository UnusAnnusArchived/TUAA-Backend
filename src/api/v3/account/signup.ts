...
export default function signup(app:Application) {
  app.use('/v3/account/signup', express.json())

  app.post('/v3/account/signup', (req, res) => {
    const users = fs.readdirSync('src/db/users')

    var body:IBody = req.body
    ...
    if (body.email && body.username && body.password && body.confirmpassword) {
      if (body.password === body.confirmpassword) {
        ...
        if (exists) {
          res.status(400).send({success:false,error:{code:1,message:'Account exists!'}})
        } else {
        ...
        res.status(400).send({success:false,error:{code:0,message:'Passwords do not match!'}})
      }
    } else {
      res.status(400).send({success:false,error:{code:2,message:'Missing info!'}})
    }
  })
}
